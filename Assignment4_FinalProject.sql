...
-- ----------------------------------------------------------creatingtables------------------------------------------------------------
				...
				CREATE TABLE regions (
					region_id 	INT (11) 	AUTO_INCREMENT 	PRIMARY KEY,    -- get increment by 1 for each new record
					region_name VARCHAR (25) 				DEFAULT NULL 	-- provide a default value for a column
				);
				...
				CREATE TABLE jobs (
                    ...
                    CHECK (NOT EXISTS (SELECT job_id FROM jobs GROUP BY job_id HAVING COUNT(*) > '20'))
				);
...
-- ---------------------------------------------------------------------- --
	...
	-- a.	Sort the rows based on the alphabetic order of substring in a column. 

				SELECT *, substr(first_name,2,4) AS ExtractString
				FROM employees
				LIMIT 10;

	-- b. another new column will show for example (Salary – average salary) 
	
				SELECT employee_id, AVG(salary) AS salAVG
				FROM  employees
				GROUP BY employee_id;
    
	-- c. Use ALTER and/or UPDATE operations to develop these columns 

				update employees set employee_id = '10' where employee_id = '100';


				ALTER TABLE employees add myCol varchar(10);


				ALTER TABLE employees Drop last_name;


			-- INSERT a set of rows INTO a new table. Use SELECT operations on other tables 
							INSERT INTO employees (first_name, last_name) 
							SELECT first_name, last_name FROM dependents
							WHERE employee_id ='11';
                                     
			-- AVG() function to calculate the average salary of each department:
			...
			-- COUNT(*) headcount of each department:
			...
			-- all employees whose salaries are greater than the highest salary of employees in the Marketing department whose id is 2:
			...
			-- all employees whose salary are not in the range of 2,500 and 2,900, you use the NOT BETWEEN operator in the WHERE clause as shown below:
			...
			-- uses the MOD function in a CASE expression to assign employees to the Odd or Even team. 
				-- The GROUP BY clause groups employees by team and the COUNT function returns the number of employees in each team.
			...
			-- use the TRUNCATE() function to remove all numbers after the decimal points from the average salary.

					SELECT department_name, TRUNCATE(AVG(salary),0) average_salary
					FROM employees e
							INNER JOIN
						departments d ON d.department_id = e.department_id
					GROUP BY department_name
			...
			-- all employees whose salaries are greater than all salaries of employees in the department 8:
			...
			-- all employees who have dependents:

					SELECT first_name, last_name
					FROM employees e
					WHERE EXISTS ( SELECT 1 dependents d WHERE d.employee_id = e.employee_id);

			-- the SUBSTRING function to extract the first character of the employee’s first name (initial) and group employees by the initial.

					SELECT SUBSTRING(first_name, 1, 1) initial, COUNT(employee_id)
					FROM employees
					GROUP BY initial;

			-- returns the top five employees with the longest names.

					SELECT employee_id, CONCAT(first_name, ' ', last_name) AS full_name, LENGTH(CONCAT(first_name, ' ', last_name)) AS len
					FROM employees
					ORDER BY len DESC
					LIMIT 5;

			-- all employees who have at least one dependent:

					SELECT employee_id, first_name, last_name
					FROM employees
					WHERE
						EXISTS( SELECT 1 FROM dependents WHERE dependents.employee_id = employees.employee_id);

			-- the employee who has the lowest salary in the company:
	
					SELECT first_name, last_name, salary, FIRST_VALUE (first_name) OVER ( ORDER BY salary ) lowest_salary
					FROM employees e;
			...
			-- how to join 3 tables: regions, countries, and locations:
...
-- ---------------------------------------------------------------------- -- 
	-- SQL as DUMPING DATA THROUGH SQL COMMAND.
-- ---------------------------------------------------------------------- -- 
